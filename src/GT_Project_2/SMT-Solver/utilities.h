...
}// puts zeros in every cell of the sudoku array
...
}//print the content of the current sudoku array
void loadConfiguration(FILE *fp, char* filePath,struct CellContent sudokuMatrix [NUM_LINE][NUM_LINE]){
    ...
    fclose(fp);
}//load a text file into the sudoku array variable
int checkViolation(struct CellContent sudokuMatrix [NUM_LINE][NUM_LINE]){
   ...
   while(end_i<=8){
    ...
    return violation;
}//checks if the current sudoku array respects the rules
...
int encodeSudoKuMatrix(){
  ...
  for (i = 0; i < NUM_LINE; i++)
      for (j = 0; j < NUM_LINE; j++){
              variable_list[i][j].i = i;
              variable_list[i][j].j = j;
              
              variable_list[i][j].var_name[0] = 'a';
              variable_list[i][j].var_name[1] = i+'1';
              variable_list[i][j].var_name[2] = j+'1';
              variable_list[i][j].var_name[3] = '\0';
              
              if(sudokuMatrix[i][j].digit)
                 variable_list[i][j].digit = sudokuMatrix[i][j].digit;
              else 
                 variable_list[i][j].digit= 0; 
